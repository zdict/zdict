    ...
    def print_using_color(self, color: Color):
        ...
        def print_examples(ex):
            def print_example_sentence(s, indent=0):
                color.print_with_highlight(
                    s,
                    r'<([a-zA-Zа-яА-Я]+)>', 'indigo', 'lindigo',
                    indent=indent
                )
...
class YandexDict(DictBase):
    ...
    API = 'https://dictionary.yandex.net/dicservice.json/queryCorpus?' \
        'ui=en&srv=tr-text&type&lang=ru-en&src={word}&dst'
